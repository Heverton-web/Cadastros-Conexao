...
export function BottomNav({ selectedModuleKey }: { selectedModuleKey?: string }) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 border-t border-border-subtle bg-header-bg/95 backdrop-blur-lg lg:hidden" style={{ paddingBottom: "env(safe-area-inset-bottom, 0px)" }}>
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={cn("flex min-h-[48px] items-center justify-center px-1 py-1 transition-colors", isActive ? "text-accent" : "text-text-muted hover:text-text-main")}
            >
              <item.icon size={18} />
            </button>
          );
