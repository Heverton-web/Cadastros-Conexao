      ...
      <div className="rounded-xl bg-card p-5 shadow-lg">
        ...
        <div className="flex flex-col gap-3 md:flex-row items-end mb-4 bg-input-bg p-3 rounded-lg border border-input-border">
          ...
          <div className="w-full md:w-32">
            <label className="text-xs text-text-muted ml-1 mb-1 block">Tipo Cliente</label>
              ...
              <div key={l.id} className="flex flex-col gap-4 rounded-xl border border-input-border bg-bg-dark p-4 transition-all hover:border-input-border/75">
                <div className="flex justify-between items-start gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <p className="text-xs text-text-muted font-mono bg-black/20 px-2 py-0.5 rounded truncate max-w-[150px] md:max-w-[300px]" title={l.token}>{l.token}</p>
                    ...
                    <div className="flex flex-col gap-1 text-xs border-r border-input-border/20 pr-2 last:border-none">
                      ...
                      <div className="flex items-center gap-1.5 mt-1">
                        {(() => {
                          ...
                          if (exp < agora) return <span className="px-2 py-0.5 rounded text-xs bg-red-500/15 text-red-400 font-bold border border-red-500/30">Expirado</span>;
                          const diffMin = (exp.getTime() - agora.getTime()) / 60000;
                          if (diffMin < 15) return <span className="px-2 py-0.5 rounded text-xs bg-yellow-500/15 text-yellow-400 font-bold border border-yellow-500/30">Expirando em Breve</span>;
                          return <span className="px-2 py-0.5 rounded text-xs bg-green-500/15 text-green-400 font-bold border border-green-500/30">Ativo</span>;
                        })()}
                      </div>
                    ...
                    <div className="flex flex-col gap-1 text-xs border-r border-input-border/20 pr-2 last:border-none">
                      ...
                      <div className="mt-1">
                        {cad.status_verificacao_token ? (
                          <span className="px-2 py-0.5 rounded text-xs bg-green-500/15 text-green-400 font-bold border border-green-500/30">Verificado & Liberado</span>
                        ) : cad["2fa_token"] ? (
                    ...
                    <div className="flex flex-col gap-1 text-xs">
                      ...
                      <div className="mt-1">
                        {cad.inicio_preenchimento ? (() => {
                          ...
                          if (restante <= 0) {
                            return <span className="px-2 py-0.5 rounded text-xs bg-red-500/15 text-red-400 font-bold border border-red-500/30 block w-max">Prazo Excedido</span>;
                          }
                          const horas = Math.floor(restante / 3600000);
                          const minutos = Math.floor((restante % 3600000) / 60000);
                          return (
                            <div className="flex flex-col gap-1">
                              <span className="px-2 py-0.5 rounded text-xs bg-blue-500/15 text-blue-400 font-bold border border-blue-500/30 w-max">Preenchimento Iniciado</span>
                              <span className="text-xs text-text-muted font-semibold">Restam: {horas}h {minutos}m</span>
                            </div>
                          );
                        })() : (
                ...
                {cad && (
                  <div className="border-t border-input-border/30 pt-3 flex flex-wrap gap-2 items-center">
                    <span className="text-xs text-text-muted font-bold uppercase tracking-wider mr-1">Simular Estados:</span>
                    
                    {/* Botões do Link Geral */}
                    <button onClick={() => handleAltExpira(cad.id, -60)} className="text-xs bg-red-500/10 text-red-400 hover:bg-red-500/25 border border-red-500/20 px-2 py-1 rounded-lg transition-colors font-medium">
                      Expirar Geral
                    </button>
                    <button onClick={() => handleAltExpira(cad.id, 10)} className="text-xs bg-yellow-500/10 text-yellow-400 hover:bg-yellow-500/25 border border-yellow-500/20 px-2 py-1 rounded-lg transition-colors font-medium">
                      Expirar em 10min
                    </button>
                    <button onClick={() => handleAltExpira(cad.id, 30 * 24 * 60)} className="text-xs bg-green-500/10 text-green-400 hover:bg-green-500/25 border border-green-500/20 px-2 py-1 rounded-lg transition-colors font-medium">
                      Resetar Validade
                    </button>

                    {/* Botões do Timer 24h */}
                    <button onClick={() => handleAltInicioPreenchimento(cad.id, -25)} className="text-xs bg-orange-500/10 text-orange-400 hover:bg-orange-500/25 border border-orange-500/20 px-2 py-1 rounded-lg transition-colors font-medium">
                      Exceder 24h
                    </button>
                    <button onClick={() => handleAltInicioPreenchimento(cad.id, -1)} className="text-xs bg-blue-500/10 text-blue-400 hover:bg-blue-500/25 border border-blue-500/20 px-2 py-1 rounded-lg transition-colors font-medium">
                      Iniciado há 1h
                    </button>

                    {/* Reset de 2FA */}
                    <button onClick={() => handleReset2FA(cad.id)} className="text-xs bg-purple-500/10 text-purple-400 hover:bg-purple-500/25 border border-purple-500/20 px-2 py-1 rounded-lg transition-colors font-medium ml-auto">
                      Reiniciar 2FA
                    </button>
                  </div>
                )}
      ...
      <div className="rounded-xl bg-card p-5 shadow-lg">
        ...
        <div className="flex flex-col gap-3 md:flex-row md:flex-wrap items-end mb-4 bg-input-bg p-3 rounded-lg border border-input-border">
          <div className="flex-1 w-full min-w-[200px]">
            <label className="text-xs text-text-muted ml-1 mb-1 block">Email Demo</label>
          ...
          <div className="w-full md:w-32">
            <label className="text-xs text-text-muted ml-1 mb-1 block">Senha Livre</label>
          ...
          <div className="w-full md:w-32">
            <label className="text-xs text-text-muted ml-1 mb-1 block">Papel</label>
          ...
          <div className="w-full md:w-20">
            <label className="text-xs text-text-muted ml-1 mb-1 block">Mocks</label>
            ...
            <div key={d.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-lg border border-input-border bg-bg-dark p-3">
              <div>
                <p className="text-xs font-bold text-text-main flex items-center gap-2">
                  {d.email_demo} <span className="text-xs font-normal bg-accent/20 text-accent px-1.5 py-0.5 rounded">{d.role_escolhida}</span>
                </p>
                <div className="flex items-center gap-2 mt-1">
                  <p className="text-xs text-text-muted font-mono">Senha: {d.senha_demo}</p>
                  <p className="text-xs text-text-muted border-l border-input-border pl-2 ml-1">{d.qtd_cadastros_mock} cadastros mockados</p>
                </div>
              </div>
