...
import { listarColaboradores, criarColaborador, deletarColaborador } from '~/features/linktree/index';
import { listarEmpresaConfig, verificarSlugDisponivel } from '~/features/linktree/services/empresa';
...
describe('LinkTree Services', () => {
  ...
  describe('listarColaboradores', () => {
    it('retorna lista quando Supabase responde com dados', async () => {
      ...
      supabase.from.mockReturnValue(
        mockQueryBuilder({
          then: vi.fn((resolve: (v: unknown) => void) => resolve({ data: [{ id: '1', nome: 'João' }], error: null })),
        })
      );
  ...
  describe('criarColaborador', () => {
    it('cria colaborador e retorna com id', async () => {
      ...
      supabase.from.mockReturnValue(
        mockQueryBuilder({
          single: vi.fn().mockResolvedValue({ data: { id: 'new-1', nome: 'Maria' }, error: null }),
        })
      );
  ...
  describe('listarEmpresaConfig', () => {
    it('retorna config quando encontrada', async () => {
      ...
      supabase.from.mockReturnValue(
        mockQueryBuilder({
          maybeSingle: vi.fn().mockResolvedValue({ data: { id: 'cfg-1', slug: 'minha-empresa' }, error: null }),
        })
      );
  ...
  describe('verificarSlugDisponivel', () => {
    ...
    it('retorna false quando slug ja existe', async () => {
      ...
      supabase.from.mockReturnValue(
        mockQueryBuilder({
          maybeSingle: vi.fn().mockResolvedValue({ data: { empresa_id: 'outra' }, error: null }),
        })
      );
