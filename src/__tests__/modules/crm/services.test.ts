...
vi.mock('@tanstack/react-start', () => ({
  createServerFn: () => {
    ...
    const chain: Record<string, any> = {
      middleware: () => chain,
      inputValidator: (fn: (x: unknown) => unknown) => { validator = fn; return chain; },
      handler: (fn: (args: { data: unknown; context: { supabase: any; userId: string } }) => unknown) => {
        return async (input: unknown) => {
          const data = validator(input);
          return fn({ data, context: { supabase: mockSupabase.supabase, userId: 'test-user' } });
        };
      },
    };
    return chain;
...
import { criarCliente, registrarVisita } from '~/features/crm/lib/visitas.functions';
...
const clienteInput = { nome_doutor: 'Dr. Teste', nome_clinica: 'Clinica Teste' };
...
describe('CRM - Visitas Server Functions', () => {
  ...
  describe('criarCliente', () => {
    it('cria cliente com sucesso', async () => {
      mockSupabase.supabase.from.mockReturnValue(
        mockQueryBuilder({ single: vi.fn().mockResolvedValue({ data: { id: 'cli-1' }, error: null }) })
      );
  ...
  describe('registrarVisita', () => {
    it('registra visita com sucesso', async () => {
      mockSupabase.supabase.from.mockReturnValue(
        mockQueryBuilder({ single: vi.fn().mockResolvedValue({ data: { id: 'vis-1' }, error: null }) })
      );
      const visitaInput = {
