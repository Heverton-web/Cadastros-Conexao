...
export async function aprovarDespesa(id: string): Promise<Despesa> {
  ...
  dispararEventoModulo(MODULO_KEY, "despesa.aprovada", { despesa_id: id }).catch(() => {});
  return despesa;
}
...
export async function reprovarDespesa(
  ...
  dispararEventoModulo(MODULO_KEY, "despesa.reprovada", { despesa_id: id, comentario }).catch(() => {});
  return despesa;
}

export async function marcarComoPaga(id: string): Promise<Despesa> {
  const despesa = await atualizarDespesa(id, { status: "paga" });
  dispararEventoModulo(MODULO_KEY, "pagamento.agendado", { despesa_id: id }).catch(() => {});
  return despesa;
}
