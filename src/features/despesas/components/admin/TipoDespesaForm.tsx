...
import { useCriarTipoDespesa, useAtualizarTipoDespesa } from "../../hooks/useTiposDespesa";
...
export function TipoDespesaForm({ open, onOpenChange, editando, empresaId }: { open: boolean; onOpenChange: (open: boolean) => void; editando: DespesaTipo | null; empresaId?: string }) {
  ...
  useEffect(() => {
    if (open) {
      reset(editando ? { nome: editando.nome, valor_maximo: editando.valor_maximo } : { nome: "", valor_maximo: 0 });
    }
  }, [open, editando, reset]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-card border-border">
        <DialogHeader>
          <DialogTitle>{editando ? "Editar Tipo de Despesa" : "Novo Tipo de Despesa"}</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="nome">Nome</Label>
            <Input id="nome" placeholder="Ex: Jantar, Almoço, Uber..." {...register("nome")} />
          ...
          <div className="space-y-2">
            ...
            <Input id="valor_maximo" type="number" step="0.01" min="0" placeholder="0.00" {...register("valor_maximo")} />
            {errors.valor_maximo && <p className="text-xs text-error">{errors.valor_maximo.message}</p>}
          </div>
          <DialogFooter>
