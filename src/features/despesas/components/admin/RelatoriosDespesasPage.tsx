...
export function RelatoriosDespesasPage() {
  ...
  const { data: despesas, isLoading } = useDespesas(filtros);
  const { data: periodos } = usePeriodos();
  const { data: tipos } = useTiposDespesa();
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      <PageHeader
        ...
        description="Visualize os dados de despesas da equipe."
        icon={BarChart3}
      />
      ...
      <div className="grid grid-cols-2 gap-2 md:gap-4">
        {[
          ...
          { label: "Aprovado", value: totalAprovado, color: "text-success" },
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
        <select
          ...
          onChange={(e) => setFiltros((f) => ({ ...f, periodo_id: e.target.value || undefined }))}
          className="rounded-lg border border-input-border bg-input-bg px-3 py-2 text-sm text-text-main flex-1"
        >
          <option value="">Todos os períodos</option>
          {periodos?.map((p) => (
            <option key={p.id} value={p.id}>{formatarData(p.data_inicio)} — {formatarData(p.data_fim)}</option>
          ))}
        </select>
        <select
