  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
        ...
        {rascunhos.length > 0 && (
          <Button onClick={() => setModalEnviar(true)} variant="outline" className="gap-1.5 w-full sm:w-auto">
            <Send size={16} /> Enviar ({rascunhos.length} — {formatarMoeda(totalRascunho)})
          </Button>
        )}
      </div>
      ...
      {envios && envios.length > 0 && (
        <div>
          <h3 className="text-xs font-semibold text-text-muted uppercase tracking-wider mb-2">Envios</h3>
          <div className="grid gap-2">
            {envios.map((envio) => (
              <div key={envio.id} className="flex items-center justify-between rounded-xl bg-card border border-border px-3 py-2.5 md:px-4 md:py-3">
              ...
              >
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className="text-sm font-medium text-text-main truncate">{d.tipo?.nome ?? "—"}</span>
                ...
                <div className="flex items-center gap-2 ml-2">
                  <span className="text-sm font-semibold text-text-main whitespace-nowrap">{formatarMoeda(d.valor)}</span>
                  <ChevronRight size={16} className="text-text-muted/40" />
                </div>
              </button>
            )) : (
              <div className="text-center py-8 text-text-muted text-sm">Nenhuma despesa lançada.</div>
            )}
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-surface-hover/30">
                  ...
                  <th className="text-center px-4 py-2.5 text-xs font-semibold text-text-muted uppercase">Comprovante</th>
                </tr>
              </thead>
              <tbody>
                {despesas && despesas.length > 0 ? despesas.map((d) => (
                  <tr key={d.id} className="border-b border-border/50 last:border-0 hover:bg-surface-hover/20 transition-colors cursor-pointer" onClick={() => setDetalhe(d)}>
                    ...
                    <td className="px-4 py-2.5 text-center">
                      {d.comprovante_url ? <Eye size={14} className="mx-auto text-accent" /> : <span className="text-text-muted/40">—</span>}
                    </td>
                  </tr>
                )) : (
                  <tr>
                    <td colSpan={6} className="px-4 py-8 text-center text-text-muted">Nenhuma despesa lançada.</td>
                  </tr>
                )}
              </tbody>
            </table>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <p className="text-sm text-text-main">{formatarData(detalhe.data_despesa)}</p>
                </div>
                <div>
                  <p className="text-[10px] font-semibold text-text-muted uppercase">Tipo</p>
                  <p className="text-sm text-text-main">{detalhe.tipo?.nome ?? "—"}</p>
                </div>
                <div>
                  <p className="text-[10px] font-semibold text-text-muted uppercase">Valor</p>
                  <p className="text-sm text-text-main font-medium">{formatarMoeda(detalhe.valor)}</p>
                </div>
                <div>
                  <p className="text-[10px] font-semibold text-text-muted uppercase">Status</p>
                  <DespesaStatusBadge status={detalhe.status} />
                </div>
              </div>
              {detalhe.descricao && (
                <div>
                  <p className="text-[10px] font-semibold text-text-muted uppercase mb-1">Descrição</p>
                  <p className="text-sm text-text-main">{detalhe.descricao}</p>
                </div>
              )}
              {detalhe.status === "reprovada" && detalhe.comentario_reprovacao && (
                <div className="rounded-lg bg-error/5 border border-error/20 p-3">
                  <p className="text-[10px] font-semibold text-error uppercase mb-1">Motivo da Reprovação</p>
                  <p className="text-sm text-text-main">{detalhe.comentario_reprovacao}</p>
                </div>
              )}
              <div>
                <p className="text-[10px] font-semibold text-text-muted uppercase mb-2">Comprovante</p>
                <ComprovanteViewer url={detalhe.comprovante_url} tipo={detalhe.comprovante_tipo} />
              </div>
            </div>
          )}
        </DialogContent>
