...
export function EnviarDespesasModal({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  const [periodoId, setPeriodoId] = useState("");
  const [confirmOpen, setConfirmOpen] = useState(false);
  const { data: periodos } = usePeriodosAbertos();
  const { data: despesas, isLoading } = useMinhasDespesas({ periodo_id: periodoId || undefined });
        ...
        <DialogContent className="bg-card border-border max-w-md">
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <label className="text-xs font-semibold text-text-muted uppercase">Período</label>
              <select
                value={periodoId}
              ...
              <>
                ...
                {isLoading ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-text-muted text-sm">Nenhuma despesa em rascunho para este período.</p>
                  </div>
                ) : (
                  <>
                      ...
                      <table className="w-full text-sm">
                        <thead className="sticky top-0 bg-surface-hover/30">
                          <tr className="border-b border-border">
                            <th className="text-left px-3 py-2 text-xs font-semibold text-text-muted uppercase">Data</th>
                        ...
                        <tbody>
                          {rascunhos.map((d) => (
                            <tr key={d.id} className="border-b border-border/50 last:border-0">
                              <td className="px-3 py-2 text-text-main">{formatarData(d.data_despesa)}</td>
                              <td className="px-3 py-2 text-text-muted">{d.tipo?.nome ?? "—"}</td>
                              <td className="px-3 py-2 text-right text-text-main font-medium">{formatarMoeda(d.valor)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                    ...
                    <div className="flex items-center justify-between p-3 rounded-xl bg-accent/5 border border-accent/20">
                      <div>
                        <p className="text-sm font-semibold text-text-main">{rascunhos.length} despesa(s)</p>
                        <p className="text-xs text-text-muted">Total: {formatarMoeda(total)}</p>
                      </div>
                    </div>
                  </>
                )}
        ...
        <AlertDialogContent className="bg-card border-border">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {rascunhos.length} despesa(s) totalizando {formatarMoeda(total)} serão enviadas para aprovação.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
            <AlertDialogAction onClick={handleEnviar}>Confirmar</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </>
