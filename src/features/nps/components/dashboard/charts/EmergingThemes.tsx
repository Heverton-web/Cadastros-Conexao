  ...
  const themes = useMemo(() => {
    ...
    return {
      rows: THEMES.map((t) => ({
        ...t,
        count: counts[t.id],
        pct: totalWithComment
          ? Math.round((counts[t.id] / totalWithComment) * 100)
          : 0,
        ...
        <CardTitle className="text-foreground text-base font-semibold flex items-center gap-2">
          ...
          <TooltipProvider delayDuration={150}>
            <Tooltip>
              ...
              >
                Cada comentário é classificado nos temas (Preço, Entrega,
                Atendimento, Produto, Pagamento, Sistema) com base em
                palavras-chave. Um mesmo comentário pode aparecer em mais de um
                tema. % é calculado sobre o total de comentários do recorte (
                {themes.totalWithComment}).
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </CardTitle>
      ...
      <CardContent className="space-y-2">
        {themes.rows.map((t) => (
          <div key={t.id} className="flex items-center gap-3">
            <span className="text-sm text-foreground w-44 truncate font-medium">
              {t.label}
            </span>
            <div className="flex-1 h-5 bg-secondary/50 rounded-md overflow-hidden">
              <div
                ...
                style={{ width: `${(t.count / themes.max) * 100}%` }}
              />
            </div>
