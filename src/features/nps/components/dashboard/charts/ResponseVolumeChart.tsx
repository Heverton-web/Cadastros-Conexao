...
const ResponseVolumeChart = ({ data }: { data: { created_at: string }[] }) => {
  ...
  return (
    <Card className="bg-gradient-to-br from-card/90 to-card/60 backdrop-blur border-border/30 shadow-lg">
      <CardHeader>
        <CardTitle className="text-foreground text-base font-semibold">Volume de Respostas (últimos 30 dias)</CardTitle>
      </CardHeader>
      <CardContent>
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={volumeData}>
            <CartesianGrid strokeDasharray="3 3" stroke="hsl(217,33%,20%)" />
            <XAxis dataKey="day" stroke="hsl(215,20%,55%)" fontSize={11} angle={-45} textAnchor="end" height={50} />
            ...
            />
            <Bar dataKey="count" radius={[4, 4, 0, 0]} fill="hsl(210,50%,55%)" fillOpacity={0.6} />
          </BarChart>
        </ResponsiveContainer>
      </CardContent>
    </Card>
