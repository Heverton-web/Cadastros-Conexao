  ...
  return (
    <div className="bg-gradient-to-br from-card/90 to-card/60 backdrop-blur border border-red-500/20 rounded-lg shadow-lg p-4 md:p-5">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-foreground text-sm font-semibold">Alertas — Detratores Recentes</h3>
        <span className="text-xs text-muted-foreground ml-auto">{detractors.length} mais recente{detractors.length === 1 ? '' : 's'}</span>
      </div>
        ...
        {detractors.map(r => (
          ...
          >
            ...
            <p className="text-xs text-foreground/90 line-clamp-3 leading-snug flex-1">
              {r.nps_comment || <span className="text-muted-foreground italic">Sem comentário</span>}
            </p>
          </div>
        ))}
      </div>
