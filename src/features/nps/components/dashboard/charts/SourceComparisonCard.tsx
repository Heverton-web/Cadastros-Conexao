...
const SourceComparisonCard = ({ data }: { data: SurveyResponse[] }) => {
  const sourceData = useMemo(() => {
    const sources: Record<string, { npsScores: number[]; matrixSums: number[]; count: number }> = {};

    data.forEach(r => {
      const src = r.source || 'Sem fonte';
      if (!sources[src]) sources[src] = { npsScores: [], matrixSums: [], count: 0 };
      sources[src].count++;
      ...
      if (vals.length) sources[src].matrixSums.push(vals.reduce((a, b) => a + b, 0) / vals.length);
    });
    ...
    return Object.entries(sources)
      ...
      .map(([source, { npsScores, matrixSums, count }]) => {
        ...
        const avgMatrix = matrixSums.length ? Number((matrixSums.reduce((a, b) => a + b, 0) / matrixSums.length).toFixed(1)) : 0;
  ...
  return (
    <Card className="bg-gradient-to-br from-card/90 to-card/60 backdrop-blur border-border/30 shadow-lg">
      <CardHeader>
        <CardTitle className="text-foreground text-base font-semibold">Comparativo por Fonte/Canal</CardTitle>
      </CardHeader>
      <CardContent>
        <ResponsiveContainer width="100%" height={220}>
          <BarChart data={sourceData}>
            ...
            />
            <Bar dataKey="nps" name="NPS" fill="hsl(38,50%,50%)" radius={[4, 4, 0, 0]} />
            <Bar dataKey="media" name="Média Critérios" fill="hsl(210,50%,55%)" radius={[4, 4, 0, 0]} fillOpacity={0.6} />
          </BarChart>
        </ResponsiveContainer>
        <div className="flex gap-4 justify-center mt-3 text-xs text-muted-foreground">
          {sourceData.map(s => (
            <span key={s.source}>📊 {s.source}: {s.respostas} respostas</span>
          ))}
        </div>
      </CardContent>
    </Card>
