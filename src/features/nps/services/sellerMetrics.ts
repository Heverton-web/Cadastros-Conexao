...
export function calcularMetricasVendedor(rows: any[]): SellerMetric[] {
  ...
  return Object.entries(groups)
    .map(([vendor, items]) => {
      const scored = items.filter((r) => r.nps_score !== null && r.nps_score !== undefined);
      ...
      MATRIX_KEYS.forEach(({ key }) => {
        const vals = items.map((r) => r[key]).filter((v: number) => typeof v === "number" && v > 0);
      ...
      return { vendor, total: items.length, nps, promoters, passives, detractors, matrixAvg, criteria };
    })
