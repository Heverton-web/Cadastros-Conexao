...
export function CollectionCard({
  ...
  const title =
    collection.title?.[language] || collection.title?.["pt-br"] || "Sem título";
  const description =
    collection.description?.[language] ||
    collection.description?.["pt-br"] ||
    "";
  const progress =
    ...
    >
      <div className="p-6 relative z-10 flex flex-col flex-1">
        <div className="flex justify-between items-start mb-4">
          <div className="icon-box-lg group-hover:scale-110 transition-transform duration-500">
            <BookOpen size={24} />
          </div>
          <span
            className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 border border-white/5 rounded-lg backdrop-blur-sm"
            style={{
              backgroundColor: colorMix(
                "var(--color-surface)",
                50,
        ...
        <div className="mt-auto">
            ...
            <div className="mb-3">
              <div className="flex justify-between text-xs font-medium mb-1">
                <span style={{ color: "var(--color-text-muted)" }}>
                  {completedCount} de {itemCount} concluídos
                </span>
          ...
          >
            ...
            {progress === 100 && itemCount > 0 ? (
              ...
              <span
                className="text-xs font-bold transition-all group-hover:translate-x-1"
