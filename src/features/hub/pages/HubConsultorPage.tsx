...
import { Search, Grid, FileText, Image as ImageIcon, Video, BookOpen, Tag, Star, Headphones, Globe } from "lucide-react";
...
export function HubConsultorPage() {
  ...
  const filtered = useMemo(() => materials.filter((m) => {
    ...
    if (!m.allowed_roles?.includes("consultant")) return false;
  ...
  const counts = useMemo(() => {
    const base = materials.filter((m) => m.active && m.allowed_roles?.includes("consultant"));
      ...
      <div className="flex-1 min-w-0 z-0">
        <div className="mb-6 sm:mb-10 relative group rounded-[1.5rem] sm:rounded-[2rem] overflow-hidden animate-fade-in">
          ...
          <div className="relative z-10 p-5 sm:p-8 md:p-10 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6">
            <div>
              <h2 className="text-2xl sm:text-4xl md:text-5xl font-bold tracking-tight mb-3" style={{ color: "var(--color-text-main)" }}>Materiais — Consultor</h2>
              <p className="text-sm sm:text-base max-w-lg font-medium" style={{ color: "var(--color-text-muted)" }}>Conteúdos disponíveis para consultores da empresa.</p>
            </div>
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 rounded-[2rem] border border-white/5" style={{ backgroundColor: colorMix("var(--color-surface)", 20, "rgba(30,41,59,0.2)") }}>
            ...
            <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>Materiais para consultores serão exibidos aqui.</p>
          </div>
        ) : (
