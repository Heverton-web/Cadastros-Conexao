...
export function HubDashboardPage({
  ...
  const [activeView, setActiveView] = useState<"materials" | "collections">(
    "materials",
  );
  ...
  const levelProgress =
    nextThreshold > 0
      ? Math.min(100, Math.round((userPoints / nextThreshold) * 100))
      : 100;
    ...
    () =>
      materials.filter((m) => {
        if (
          roleMapped &&
          m.allowed_roles &&
          !m.allowed_roles.includes(roleMapped)
        )
          return false;
  ...
  const MenuCategory = ({
    ...
    >
      <div className="flex items-center gap-2 md:gap-4 relative z-10">
        <div
          className={`icon-box transition-all duration-300 ${active ? "!bg-white/10 !border-transparent" : ""}`}
        >
          <Icon size={16} />
        </div>
        <span
          className={`text-xs md:text-sm tracking-wide whitespace-nowrap ${active ? "font-bold" : "font-medium"}`}
          style={active ? { color: "var(--color-text-main)" } : {}}
        >
          {label}
        </span>
      </div>
      <span
        className={`text-[10px] font-bold px-2 py-0.5 md:px-2.5 md:py-1 rounded-lg transition-all duration-300 ${active ? "bg-white/10 backdrop-blur-sm" : "border"}`}
        style={
          active
      ...
      <aside className="w-full md:w-72 shrink-0 z-30">
        <div className="sticky top-28 space-y-4 animate-slide-up">
          {isSuperAdmin && empresas.length > 0 && (
            <div
              className="rounded-2xl p-3 border border-white/10 flex items-center gap-2"
              style={{
          ...
          {activeView === "materials" && (
            <div
              className="backdrop-blur-xl border border-white/10 p-2 sm:p-3 rounded-3xl flex flex-row md:flex-col overflow-x-auto md:overflow-visible gap-1.5 sm:gap-2 no-scrollbar shadow-xl shadow-black/5"
              style={{
                backgroundColor: colorMix(
                  "var(--color-surface)",
                  30,
            ...
            >
              ...
              <div className="hidden md:block mt-4 pt-4 px-2 border-t border-white/5">
                <div
                  className="relative overflow-hidden rounded-2xl p-5 group transition-all duration-300 hover:shadow-lg"
                  style={{
                    background: `linear-gradient(to br, ${colorMix("var(--color-warning)", 10, "rgba(234,179,8,0.1)")}, ${colorMix("var(--color-warning)", 5, "rgba(234,179,8,0.05)")})`,
                    border: `1px solid ${colorMix("var(--color-warning)", 20, "rgba(234,179,8,0.2)")}`,
                  }}
                >
                  <div className="relative z-10">
                    ...
                    >
                      Use{" "}
                      <span
                        className="font-mono bg-white/10 px-1 rounded text-[10px]"
        ...
        {activeView === "materials" && (
          <>
            <div className="mb-6 sm:mb-10 relative group rounded-[1.5rem] sm:rounded-[2rem] overflow-hidden animate-fade-in">
              ...
              />
              <div
                className="absolute -right-20 -bottom-40 w-96 h-96 rounded-full blur-[100px] animate-pulse"
                style={{
                  backgroundColor: colorMix(
                    "var(--color-accent)",
                    20,
              ...
              <div className="relative z-10 p-5 sm:p-8 md:p-10 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6 sm:gap-8 backdrop-blur-sm">
                <div>
                  <h2
                    className="text-2xl sm:text-4xl md:text-5xl font-bold tracking-tight mb-3 drop-shadow-sm"
                    style={{ color: "var(--color-text-main)" }}
                  >
                    Materiais
                  </h2>
                  <p
                    className="text-sm sm:text-base max-w-lg leading-relaxed font-medium"
                    style={{ color: "var(--color-text-muted)" }}
                  >
                    Explore, visualize e baixe todos os materiais disponíveis
                    para o seu perfil.
                  </p>
                </div>
                <div className="relative w-full xl:w-96">
                  <div
                    className="relative backdrop-blur-xl border border-white/10 rounded-2xl flex items-center shadow-inner transition-all duration-300"
                    style={{
                      backgroundColor: colorMix(
                        "var(--color-surface)",
                        60,
                  ...
                  >
                    ...
                    <input
                      ...
                      className="w-full bg-transparent border-none py-3 sm:py-4 px-3 sm:px-4 focus:ring-0 text-sm font-medium outline-none"
                      style={{ color: "var(--color-text-main)" }}
                      value={searchTerm}
            ...
            {isLoading ? (
              ...
              <div
                className="flex flex-col items-center justify-center py-24 backdrop-blur-sm border border-white/5 rounded-[2rem] animate-fade-in text-center px-4"
                style={{
                  backgroundColor: colorMix(
                    "var(--color-surface)",
                    20,
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6 pb-6">
                {filteredMaterials.map((mat, index) => (
                  ...
                  >
                    <MaterialCard
                      material={mat}
                      progress={progress.find((p) => p.material_id === mat.id)}
                    />
                  </div>
                ))}
              </div>
        ...
        {activeView === "collections" && (
          <>
            ...
            {collections.length === 0 ? (
              <div
                className="flex flex-col items-center justify-center py-24 rounded-[2rem] text-center px-4 border border-white/5"
                style={{
                  backgroundColor: colorMix(
                    "var(--color-surface)",
                    20,
              ...
              >
                ...
                <p
                  ...
                  style={{ color: "var(--color-text-muted)" }}
                >
                  As trilhas de aprendizagem serão exibidas aqui.
                </p>
              </div>
            ) : (
