...
export async function listarRecorrencias(funilId: string): Promise<Recurring[]> {
...
export async function listarRecorrenciasTarefa(tarefaId: string): Promise<Recurring[]> {
  const { data, error } = await supabase
    .from("funis_recurring")
    .select("*")
...
export async function criarRecorrencia(input: RecurringInput): Promise<Recurring> {
...
export async function atualizarRecorrencia(id: string, input: Partial<RecurringInput>): Promise<Recurring> {
  const updates: Record<string, unknown> = {};
  if (input.frequencia) updates.frequencia = input.frequencia;
  if (input.config) {
    updates.config = input.config;
    updates.proxima_exec = calcularProximaExec(input.frequencia ?? "diaria", input.config);
  }
...
export async function toggleRecorrencia(id: string, ativo: boolean): Promise<void> {
  const { error } = await supabase
    .from("funis_recurring")
    .update({ ativo })
    .eq("id", id);
...
function calcularProximaExec(frequencia: string, config: Record<string, unknown>): string {
  const now = new Date();
