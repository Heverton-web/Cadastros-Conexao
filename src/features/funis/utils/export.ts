...
export function exportToCSV(funil: Funil): string {
  ...
  const headers = ["Título","Descrição","Prioridade","Coluna","Responsável","Data Início","Data Fim","Status","Criado em"];
  const rows = tarefas.filter((t) => !t.parent_task_id).map((t) => [
    esc(t.titulo), esc(t.descricao ?? ""), esc(t.prioridade), esc(colunaMap.get(t.coluna_id) ?? ""),
    esc(t.atribuido_para ?? ""), esc(t.data_inicio ? new Date(t.data_inicio).toLocaleDateString("pt-BR") : ""),
    esc(t.data_fim ? new Date(t.data_fim).toLocaleDateString("pt-BR") : ""), esc(t.completed_at ? "Concluída" : "Em andamento"),
...
export function exportToJSON(funil: Funil): string {
  return JSON.stringify({
    funil: { id: funil.id, titulo: funil.titulo, descricao: funil.descricao, created_at: funil.created_at },
    colunas: funil.colunas?.map((c) => ({ id: c.id, titulo: c.titulo, posicao: c.posicao })),
    tarefas: funil.tarefas?.map((t) => ({ id: t.id, titulo: t.titulo, descricao: t.descricao, prioridade: t.prioridade, coluna_id: t.coluna_id, atribuido_para: t.atribuido_para, data_inicio: t.data_inicio, data_fim: t.data_fim, completed_at: t.completed_at, tools: t.tools, custom_fields: t.custom_fields })),
  }, null, 2);
}
...
export function exportToPDF(funil: Funil): void {
  ...
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${funil.titulo}</title><style>
    ...
    ${colunas.map((col) => `<h3>${col.titulo}</h3><table><thead><tr><th>Tarefa</th><th>Prioridade</th><th>Status</th><th>Prazo</th></tr></thead><tbody>${tarefas.filter((t) => t.coluna_id === col.id && !t.parent_task_id).map((t) => `<tr class="${t.completed_at ? "completed" : ""}"><td>${t.titulo}</td><td>${t.prioridade}</td><td>${t.completed_at ? "Concluída" : "Em andamento"}</td><td>${t.data_fim ? new Date(t.data_fim).toLocaleDateString("pt-BR") : "—"}</td></tr>`).join("")}</tbody></table>`).join("")}
