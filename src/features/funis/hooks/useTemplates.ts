...
import { listarTemplates, criarTemplate, atualizarTemplate, deletarTemplate, aplicarTemplate } from "../services/templates";
...
export function useCriarTemplate() {
  ...
  return useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["funis-templates"] }),
  });
}
...
export function useAtualizarTemplate() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, input }: { id: string; input: Partial<TemplateInput> }) => atualizarTemplate(id, input),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["funis-templates"] }),
  });
}
...
export function useAplicarTemplate() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ templateId, funilId }: { templateId: string; funilId: string }) => aplicarTemplate(templateId, funilId),
