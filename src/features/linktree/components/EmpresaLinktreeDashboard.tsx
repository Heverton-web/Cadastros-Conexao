  ...
  if (isSuper && !selectedEmpresaId) {
    return (
      <div className="space-y-6">
        ...
        <p className="text-sm text-muted-foreground">Selecione uma empresa para gerenciar o linktree.</p>
  ...
  return (
    <div className="space-y-6">
      <header className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-text-main">Linktree da Empresa</h1>
          <p className="mt-1 text-sm text-muted-foreground">{config.bio || "Gerencie os links do seu linktree."}</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={() => setQrOpen(true)}><QrCode className="size-4" /> QR Code</Button>
          <Button variant="outline" onClick={() => window.open(publicUrl, "_blank")}><ExternalLink className="size-4" /> Ver pagina</Button>
          <Button onClick={() => navigate({ to: "/linktree/empresa/editor" })}><Settings className="size-4" /> Editor</Button>
        </div>
      </header>
      ...
      <div className="flex gap-2">
        <button onClick={() => setTab("links")} className={`rounded-md px-4 py-2 text-sm font-medium transition ${tab === "links" ? "bg-primary text-primary-foreground" : "bg-surface-hover text-muted-foreground"}`}>Links</button>
        <button onClick={() => setTab("analytics")} className={`flex items-center gap-2 rounded-md px-4 py-2 text-sm font-medium transition ${tab === "analytics" ? "bg-primary text-primary-foreground" : "bg-surface-hover text-muted-foreground"}`}><BarChart3 className="size-4" /> Analytics</button>
      </div>

      {tab === "links" ? <LinksList sections={sections} links={links} empresaId={selectedEmpresaId} /> : <AnalyticsPanel empresaId={selectedEmpresaId} />}
