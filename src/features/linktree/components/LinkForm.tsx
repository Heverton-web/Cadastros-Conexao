...
export function LinkForm({ sections, link, onSave, onCancel, saving }: Props) {
  ...
  const [sectionId, setSectionId] = useState(link?.section_id ?? sections[0]?.id ?? "");
  const [agendadoInicio, setAgendadoInicio] = useState(link?.agendado_inicio?.slice(0, 16) ?? "");
  const [agendadoFim, setAgendadoFim] = useState(link?.agendado_fim?.slice(0, 16) ?? "");
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="space-y-1.5">
        <Label>Titulo *</Label>
        <Input value={titulo} onChange={(e) => setTitulo(e.target.value)} placeholder="Meu link" required />
      </div>
      ...
      <div className="flex gap-2 pt-2">
        <Button type="submit" disabled={saving || !titulo.trim() || !url.trim()} className="flex-1">
