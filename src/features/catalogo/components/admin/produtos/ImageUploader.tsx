...
import type { ProdutoTipoImagem, FonteImagem } from "~/features/catalogo/types"
...
export function ImageUploader({ produtoTipo, produtoSku, empresaId }: Props) {
  ...
  async function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const res = await fetch(uploadUrl, {
      method: "POST",
      headers: { "apikey": SERVICE_ROLE_KEY, "Authorization": `Bearer ${SERVICE_ROLE_KEY}`, "Content-Type": file.type || "image/png" },
      body: file,
    })
  ...
  async function addUrl() {
    ...
    console.log("[ImageUploader] INSERT URL:", { empresa_id: empresaId, produto_tipo: produtoTipo, produto_sku: produtoSku, url: urlInput.trim(), fonte: f })

    if (imagemId) {
      const { error } = await supabase.from("catalogo_imagens_produto").update({ url_imagem: urlInput.trim(), fonte: f }).eq("id", imagemId)
      if (error) { console.error("[ImageUploader] UPDATE ERROR:", error); toast.error(error.message); return }
    } else {
      const { data, error } = await supabase.from("catalogo_imagens_produto").insert({
        empresa_id: empresaId, produto_tipo: produtoTipo, produto_sku: produtoSku,
        url_imagem: urlInput.trim(), fonte: f, ordem_exibicao: 0,
      }).select("id").single()
      if (error) { console.error("[ImageUploader] INSERT ERROR:", error); toast.error(error.message); return }
      if (data) setImagemId(data.id)
    }
    ...
    <div className="rounded-xl bg-[var(--color-surface)] border border-white/5 p-4 space-y-4">
      ...
      {tipoAnexo === "arquivo" && (
        <div>
          ...
          {uploading && <p className="text-xs text-[#c9a655] mt-1">Enviando...</p>}
        </div>
      )}
      ...
      {(tipoAnexo === "url_s3" || tipoAnexo === "url_gdrive") && (
        <div className="space-y-2">
          <input type="text" value={urlInput} onChange={e => setUrlInput(e.target.value)} className={inputCls}
            placeholder={tipoAnexo === "url_s3" ? "https://s3.amazonaws.com/..." : "https://drive.google.com/..."} />
