...
export const dashboardModule: ModuleDefinition = {
  ...
  events: [],
  setup: () => {
    for (const p of DASHBOARD_PERMISSIONS) registerPermission({ key: p.key, label: p.label, description: p.description, group: p.group });
    registerNavItem({ id: "mktg-dashboard", label: "Dashboard", icon: BarChart3, to: "/marketing/dashboard", permissionCheck: (perms) => perms?.mktg_dashboard_ver === true, order: 10, moduloKey: "marketing" });
