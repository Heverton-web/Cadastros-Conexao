    ...
    mutationFn: async (payload: Partial<MapasDistributor>) => {
      if (payload.id) {
        const { data } = await supabase
          ...
          .select()
          .single();
        dispararEventoModulo("mapas", "distribuidor.atualizado", { distribuidor_id: data.id, nome: data.name, empresa_id: data.empresa_id }).catch(() => {});
        return data as MapasDistributor;
      }
      const { data } = await supabase
        .from("mapas_distribuidores")
        ...
        .select()
        .single();
      dispararEventoModulo("mapas", "distribuidor.criado", { distribuidor_id: data.id, nome: data.name, empresa_id: data.empresa_id }).catch(() => {});
      return data as MapasDistributor;
    },
    onSuccess: () => {
...
export function useDeleteDistributor() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      ...
      if (dist) {
        dispararEventoModulo("mapas", "distribuidor.excluido", { distribuidor_id: id, nome: dist.name, empresa_id: dist.empresa_id }).catch(() => {});
      }
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["mapas", "distributors"] });
    ...
    mutationFn: async (payload: Partial<MapasConsultant>) => {
      if (payload.id) {
        const { data } = await supabase
          ...
          .select()
          .single();
        dispararEventoModulo("mapas", "consultor.atualizado", { consultor_id: data.id, nome: data.name, empresa_id: data.empresa_id }).catch(() => {});
        return data as MapasConsultant;
      }
      const { data } = await supabase
        .from("mapas_consultores")
        ...
        .select()
        .single();
      dispararEventoModulo("mapas", "consultor.criado", { consultor_id: data.id, nome: data.name, empresa_id: data.empresa_id }).catch(() => {});
      return data as MapasConsultant;
    },
    onSuccess: () => {
...
export function useDeleteConsultant() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      ...
      if (cons) {
        dispararEventoModulo("mapas", "consultor.excluido", { consultor_id: id, nome: cons.name, empresa_id: cons.empresa_id }).catch(() => {});
      }
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["mapas", "consultants"] });
