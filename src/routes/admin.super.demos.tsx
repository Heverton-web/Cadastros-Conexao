  ...
  return (
    <div className="p-4 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-text-muted">Gerenciamento de acessos demo</p>
          </div>
        </div>
      ...
      <div className="space-y-2">
        {demos.map((d) => (
          <div key={d.id} className="flex items-center justify-between p-3 rounded-lg bg-card border border-border-subtle">
            <div>
        ...
        {demos.length === 0 && <p className="text-center text-sm text-text-muted py-8">Nenhuma demo cadastrada.</p>}
      </div>

      {showModal && <CriarDemoModal onClose={() => setShowModal(false)} onCreated={() => { setShowModal(false); carregar(); }} />}
    </div>
  );
}

function CriarDemoModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setCreating(true);
    try {
      const modulosAtivos = Object.entries(modulos).filter(([, v]) => v).map(([k]) => k);
      ...
      const { error: insertErr } = await supabase.from("demo_credentials").insert({
        user_id: uid,
        email_demo: email,
        senha_demo: senha,
        role_escolhida: role,
      ...
      <div className="w-full max-w-lg rounded-2xl bg-card border border-border-subtle p-6 shadow-xl mt-4 mb-8">
        ...
        <form onSubmit={handleCreate} className="flex flex-col gap-4">
          ...
          <div>
            ...
            <div className="space-y-1">
              {registeredModules.map((mod) => {
                ...
                return (
                  <div key={mod.key} className="flex items-center justify-between py-1.5 px-2 rounded-lg hover:bg-surface-hover transition-colors">
                    <div className="flex items-center gap-2">
                      <mod.icon size={14} className={ativo ? "text-accent" : "text-text-muted"} />
          ...
          <button type="submit" disabled={creating}
            ...
            {creating ? <Loader2 size={16} className="animate-spin" /> : <><span>Criar Demo</span><ChevronRight size={16} /></>}
          </button>
        </form>
      </div>
    </div>
