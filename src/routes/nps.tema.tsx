...
import { salvarEmpresaConfig, listarEmpresas, buscarEmpresa, buscarEmpresaConfig } from "~/features/empresas";
import { NPS_SURVEY_DEFAULTS, SURVEY_COLOR_GROUPS } from "~/features/nps/theme";
import { Palette, Save, Loader2, Eye, X, ArrowRight, ArrowLeft, CheckCircle2 } from "lucide-react";
    ...
    <div className="flex flex-col items-center gap-6" style={cssVars}>
      ...
      <div className={`w-full max-w-md bg-[var(--nps-card-bg)] ${noBorders ? 'border-0' : 'border border-[var(--nps-card-border)]'} backdrop-blur-lg rounded-2xl p-6 md:p-8 shadow-2xl relative overflow-hidden`}>
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          />
          <div className={`h-8 w-px bg-[var(--nps-header-divider)] flex-shrink-0 ${noBorders ? 'opacity-0' : ''}`} />
...
function NpsTemaPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setLogoUrl((emp as any)?.logo_url || null);
      if (config?.theme) {
