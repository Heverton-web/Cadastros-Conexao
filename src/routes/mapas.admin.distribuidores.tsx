...
function MapasAdminDistribuidoresPage() {
  ...
  const [editing, setEditing] = useState<Partial<MapasDistributor> | null>(null);
  ...
  const rows = useMemo(() => {
    ...
    return (distQ.data ?? []).filter((r) =>
      !s || r.name.toLowerCase().includes(s) || (r.city ?? "").toLowerCase().includes(s) || (r.code ?? "").toLowerCase().includes(s),
    );
  }, [distQ.data, search]);
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-4 p-6">
      ...
      <div className="relative">
        ...
        <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar por nome, cidade ou código…" className="pl-10" />
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-surface bg-card">
        <div className="hidden md:grid md:grid-cols-[1fr_120px_140px_100px_140px_120px] gap-2 border-b border-surface bg-surface/40 px-4 py-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">
          <span>Nome</span><span>Código</span><span>Cidade</span><span>UF</span><span>Categoria</span><span className="text-right">Ações</span>
        </div>
        {distQ.isLoading ? (
          ...
          <ul className="divide-y divide-surface">
            {rows.map((r) => (
              <li key={r.id} className="grid grid-cols-1 gap-1 px-4 py-3 md:grid-cols-[1fr_120px_140px_100px_140px_120px] md:items-center">
                ...
                <span className="text-sm">{r.category === "EXCLUSIVE" ? "Exclusivo" : "Não-exclusivo"}</span>
              ...
              <div className="grid grid-cols-[120px_1fr] gap-3">
                ...
                <fieldset>
                  ...
                  <Input
                    ...
                    placeholder={loadingCities ? "Carregando..." : "Digite a cidade..."}
                  />
