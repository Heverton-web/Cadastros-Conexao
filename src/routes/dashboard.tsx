...
import { Loader2, CheckCircle, XCircle, Link2, Clock, AlertTriangle, HelpCircle, Users, ArrowUpRight, BarChart3, TrendingUp, UserPlus } from "lucide-react";
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-accent/20 via-accent/10 to-transparent border border-accent/20 p-5 transition-all duration-300 hover:shadow-lg hover:shadow-accent/10 hover:border-accent/40">
            ...
            <p className="text-xs font-semibold text-accent/80 uppercase tracking-wider">Total</p>
            <p className="text-4xl font-bold text-text-main mt-2">{stats.total}</p>
          ...
          <div className="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-yellow-500/20 via-yellow-500/10 to-transparent border border-yellow-500/20 p-5 transition-all duration-300 hover:shadow-lg hover:shadow-yellow-500/10 hover:border-yellow-500/40">
            ...
            <p className="text-xs font-semibold text-yellow-400/80 uppercase tracking-wider">Pendentes</p>
            <p className="text-4xl font-bold text-text-main mt-2">{pendentes}</p>
          ...
          <div className="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-green-500/20 via-green-500/10 to-transparent border border-green-500/20 p-5 transition-all duration-300 hover:shadow-lg hover:shadow-green-500/10 hover:border-green-500/40">
            ...
            <p className="text-xs font-semibold text-green-400/80 uppercase tracking-wider">Aprovados</p>
            <p className="text-4xl font-bold text-text-main mt-2">{stats.aprovados}</p>
          ...
          <div className="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-500/20 via-blue-500/10 to-transparent border border-blue-500/20 p-5 transition-all duration-300 hover:shadow-lg hover:shadow-blue-500/10 hover:border-blue-500/40">
            ...
            <p className="text-xs font-semibold text-blue-400/80 uppercase tracking-wider">Taxa Aprovação</p>
            <p className="text-4xl font-bold text-text-main mt-2">{taxaAprovacao}%</p>
            <div className="mt-2 h-1.5 w-full rounded-full bg-blue-500/10 overflow-hidden">
              <div className="h-full rounded-full bg-gradient-to-r from-blue-500 to-blue-400 transition-all duration-1000" style={{ width: `${taxaAprovacao}%` }} />
            </div>
          </div>
        </div>
      )}
      ...
      {!loading && (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {[
            { label: "Links", value: stats.link_gerado, icon: Link2, color: "text-blue-400", bg: "bg-blue-500/10", border: "border-blue-500/20" },
            { label: "Enviados", value: stats.dados_enviados, icon: Clock, color: "text-cyan-400", bg: "bg-cyan-500/10", border: "border-cyan-500/20" },
            { label: "Análise", value: stats.em_analise, icon: AlertTriangle, color: "text-yellow-400", bg: "bg-yellow-500/10", border: "border-yellow-500/20" },
            { label: "Correção", value: stats.em_correcao, icon: AlertTriangle, color: "text-orange-400", bg: "bg-orange-500/10", border: "border-orange-500/20" },
            { label: "Aprovados", value: stats.aprovados, icon: CheckCircle, color: "text-green-400", bg: "bg-green-500/10", border: "border-green-500/20" },
            { label: "Reprovados", value: stats.reprovados, icon: XCircle, color: "text-red-400", bg: "bg-red-500/10", border: "border-red-500/20" },
          ].map((item) => (
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-bold text-text-main">Solicitações Recentes</h2>
          <Link to="/clientes" className="flex items-center gap-1 text-sm text-accent hover:text-accent-hover transition-colors font-medium">
            ...
            {recentes.map((c, i) => (
              ...
              >
                ...
                <div className="flex flex-col items-end gap-1 shrink-0">
                  <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-semibold ${STATUS_COLOR[c.status]}`}>
