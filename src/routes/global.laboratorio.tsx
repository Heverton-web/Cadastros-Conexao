...
function AdminLaboratorio() {
  const [links, setLinks] = useState<
    { descricao: string; tipo: string; url: string }[]
  >([]);
  ...
  function gerarLink() {
    ...
    setLinks((p) => [
      ...p,
      {
        ...
        tipo,
        url: `${window.location.origin}/pre-cadastro/${crypto.randomUUID()}`,
      },
    ]);
    setDescricao("");
  }
        ...
        <div className="flex flex-col gap-3">
          {links.length === 0 ? (
            ...
            links.map((link, i) => (
              <div
                key={i}
                className="rounded-lg bg-input-bg p-3 border border-border-subtle flex items-center gap-3"
              >
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-text-muted mt-0.5">
                    {link.tipo === "PF" ? "Física" : "Jurídica"}
                  </p>
                  <code className="text-xs text-accent font-mono block truncate mt-1">
                    {link.url}
                  </code>
                </div>
                <button
                  onClick={() => navigator.clipboard.writeText(link.url)}
                  className="shrink-0 rounded-lg bg-accent/10 px-2.5 py-1.5 text-xs font-medium text-accent hover:bg-accent/20 transition-colors"
                >
                  Copiar
                </button>
              </div>
