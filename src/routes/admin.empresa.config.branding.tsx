...
import { buscarEmpresaConfig, salvarEmpresaConfig, uploadEmpresaLogo, deletarEmpresaLogo, listarEmpresas, type Empresa, type EmpresaConfig } from "~/lib/empresas";
import { useState, useEffect } from "react";
import { Image, Save, Loader2, ArrowLeft, Upload, Trash2, Building2 } from "lucide-react";
import toast from "react-hot-toast";

export const adminEmpresaConfigBrandingRoute = createRoute({
  getParentRoute: () => authLayout,
  path: "/admin/empresa/config/branding",
      ...
      <div className="rounded-xl bg-input-bg p-4">
        ...
        <div className="flex items-start gap-4">
          <div className="w-24 h-24 rounded-lg border border-border-subtle bg-white flex items-center justify-center overflow-hidden shrink-0">
            {url ? <img src={url} className="w-full h-full object-contain" alt={label} /> : <Image size={24} className="text-text-muted/40" />}
          </div>
          <div className="flex-1 min-w-0 space-y-2">
            ...
            <div className="flex gap-2">
              <button onClick={() => handleUpload(tipo)} disabled={uploading === tipo}
                ...
                {uploading === tipo ? <Loader2 size={12} className="animate-spin" /> : <Upload size={12} />} Upload
              </button>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="p-4 max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-lg font-bold text-text-main flex items-center gap-2"><Image size={18} className="text-accent" /> Branding</h1>
      </div>
