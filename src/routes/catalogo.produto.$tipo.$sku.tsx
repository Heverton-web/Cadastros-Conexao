...
import { addToCart, formatBRL, getPrecoFromDB, resolveBOMItem } from "~/features/catalogo/services/carrinho.service"
...
import { ArrowLeft, ShoppingCart, Box, Zap, ExternalLink, Check } from "lucide-react"
...
function ProdutoPage() {
  ...
  const backTo = () => {
    if (tipo === 'implante') return navigate({ to: '/catalogo/implantes', search: { conexao: search.conexao, familia: search.familia, linha: search.linha } })
    if (tipo === 'abutment') return navigate({ to: '/catalogo/componentes', search: { tipoReab: search.tipoReab, familia: search.familia, tipoAbutment: search.tipoAbutment } })
...
function ProductHeader({ cor, badge, nome, sku }: { cor: string; badge?: string; nome: string; sku: string }) {
  return (
    <div className="space-y-4">
      {badge && (
        <div className="inline-flex items-center px-4 py-1.5 rounded-full border" style={{ borderColor: cor, backgroundColor: `${cor}12`, color: cor }}>
...
function SpecCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="p-4 rounded-xl bg-[var(--color-surface)]/60 border border-[var(--color-border-subtle)]">
...
function ImplanteDetail({ sku }: { sku: string }) {
  ...
  const hasProtocolo = protocolos && protocolos.length > 0

  const tabs = [
    { key: "ficha", label: "Ficha Técnica" },
    ...(hasProtocolo ? [{ key: "fresagem", label: "Protocolo de Fresagem", count: protocolos!.length }] : []),
  ]
        ...
        {activeTab === "ficha" && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              ...
              {impl.rosca_interna && <SpecCard label="Rosca Interna" value={impl.rosca_interna} />}
              {impl.torque_insercao && <SpecCard label="Torque Max" value={`${impl.torque_insercao} N·cm`} />}
              {impl.regiao_apical && <SpecCard label="Região Apical" value={impl.regiao_apical} />}
              {impl.regiao_cervical && <SpecCard label="Região Cervical" value={impl.regiao_cervical} />}
            </div>
          </div>
        )}

        {/* Protocolo de Fresagem */}
        {activeTab === "fresagem" && hasProtocolo && (
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
      ...
      <div className="lg:col-span-8 xl:col-span-7 space-y-8">
        ...
        {activeTab === "ficha" && (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {ab.diametro_plataforma && <SpecCard label="Plataforma" value={`${ab.diametro_plataforma} mm`} />}
            {ab.angulacao_graus != null && <SpecCard label="Angulação" value={`${ab.angulacao_graus}°`} />}
            {ab.altura_transmucoso != null && <SpecCard label="Transmucoso" value={`${ab.altura_transmucoso} mm`} />}
            {ab.altura_corpo != null && <SpecCard label="Corpo" value={`${ab.altura_corpo} mm`} />}
            {ab.torque_ncm != null && <SpecCard label="Torque" value={`${ab.torque_ncm} Ncm`} />}
          </div>
        )}
...
function KitDetail({ sku }: { sku: string }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
      ...
      <div className="lg:col-span-8 xl:col-span-7 space-y-8">
        <div className="space-y-4">
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black leading-[0.95] text-white tracking-tighter">{kit.nome}</h1>
          <p className="font-mono text-sm text-[var(--color-text-muted)]">SKU: <span className="text-white/80">{kit.sku}</span></p>
          {kit.descricao && <p className="text-sm text-[var(--color-text-muted)] leading-relaxed max-w-lg">{kit.descricao}</p>}
        </div>
...
function PromocionalDetail({ id }: { id: string }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
      ...
      <div className="lg:col-span-8 xl:col-span-7 space-y-8">
        <div className="space-y-4">
          <div className="inline-flex items-center px-4 py-1.5 rounded-full border border-[var(--color-accent)] bg-[var(--color-accent)]/10 text-[var(--color-accent)]">
        ...
        <div className="p-6 rounded-2xl bg-[var(--color-surface)]/40 border border-[var(--color-border-subtle)] backdrop-blur-md">
