...
import { ArrowLeft, Settings, Shield, Key, Bell, FlaskConical, Zap, FormInput, Link as LinkIcon } from "lucide-react";
import { useState } from "react";

export const adminSuperModuloDetailRoute = createRoute({
  getParentRoute: () => authLayout,
  path: "/admin/super/modulos/$key",
  component: AdminSuperModuloDetail,
});

type TabKey = "geral" | "permissoes" | "credenciais" | "eventos" | "laboratorio" | "acoes" | "formularios" | "apis";
...
function AdminSuperModuloDetail() {
  ...
  if (!mod) {
    return (
      <div className="p-4 max-w-4xl mx-auto">
        <button onClick={() => navigate({ to: "/admin/super/modulos" })} className="flex items-center gap-1 text-sm text-accent mb-4">
  ...
  return (
    <div className="p-4 max-w-4xl mx-auto">
      <button onClick={() => navigate({ to: "/admin/super/modulos" })} className="flex items-center gap-1 text-sm text-accent mb-4 hover:underline">
      ...
      {tab === "laboratorio" && <PlaceholderTab title="Laboratório" desc="Testes e experimentos do módulo." />}
      {tab === "acoes" && <PlaceholderTab title="Ações" desc="Ações customizadas do módulo." />}
      {tab === "formularios" && <PlaceholderTab title="Formulários" desc="Campos e formulários dinâmicos do módulo." />}
      {tab === "apis" && <PlaceholderTab title="APIs" desc="Conectores de API do módulo." />}
    </div>
  );
}
    ...
    <div className="space-y-4">
      ...
      <Section title="Ambientes">
        <div className="flex flex-wrap gap-2">
          {mod.ambientes.map((amb) => (
            <span key={amb} className="px-2.5 py-1 rounded-full bg-accent/10 text-accent text-xs font-medium">
              {amb}
            </span>
          ))}
        </div>
      ...
      <Section title="Rotas">
        <div className="flex flex-col gap-1">
          {mod.routes.map((r) => (
            <span key={r} className="text-xs font-mono text-text-muted bg-input-bg px-2 py-1 rounded">
              {r}
            </span>
          ))}
        </div>
      ...
      <Section title="Feature Flags">
        <div className="flex flex-wrap gap-2">
          <FlagBadge label="Escopo de Credenciais" active={mod.hasCredentialScopes} />
      ...
      <Section title="Permissões do Módulo">
        ...
        <div className="flex flex-col gap-1">
          {mod.permissions.map((p) => (
            <div key={p} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-input-bg">
    ...
    <div className="space-y-4">
      <Section title="Credenciais com Escopo">
        ...
        <div className="flex flex-wrap gap-2 mt-2">
          {mod.abas.map((aba) => (
            <label key={aba.key} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-input-bg cursor-pointer">
  ...
  return (
    <div className="space-y-4">
      <Section title="Eventos do Módulo">
        <p className="text-xs text-text-muted mb-3">
          {mod.events.length} eventos pré-definidos. Eventos disparam webhooks configurados.
        </p>
        <div className="flex flex-col gap-2">
          {mod.events.map((evt) => (
            <div key={evt.key} className="flex items-start gap-3 p-3 rounded-lg bg-input-bg">
...
function FlagBadge({ label, active }: { label: string; active?: boolean }) {
  return (
    <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${active ? "bg-green-500/10 text-green-400 border border-green-500/20" : "bg-input-bg text-text-muted border border-border-subtle"}`}>
      {label}: {active ? "✓" : "✗"}
    </span>
  );
}
