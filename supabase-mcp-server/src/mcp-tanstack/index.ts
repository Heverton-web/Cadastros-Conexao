  ...
  tools: [
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          path: { type: 'string', description: 'Path da rota (ex: /modulo/page)' },
          component: { type: 'string', description: 'Nome do componente' },
          protected: { type: 'boolean', description: 'Se a rota é protegida', default: true },
        },
...
server.setRequestHandler('tools/call', async (request) => {
  ...
  switch (name) {
    case 'tanstack_create_route': {
      ...
      const routeDir = join(process.cwd(), 'src', 'routes', ...path.split('/').filter(Boolean))
...
`
      ...
      return { content: [{ type: 'text', text: `Rota ${path} criada com componente ${component}` }] }
    }
    case 'tanstack_list_routes': {
      ...
      const listRoutes = (dir: string, prefix = ''): string[] => {
        ...
        for (const entry of entries) {
          if (entry.isDirectory()) {
            routes.push(...listRoutes(join(dir, entry.name), `${prefix}/${entry.name}`))
          } else if (entry.name.endsWith('.tsx') && entry.name !== '_auth.tsx' && entry.name !== '__root.tsx') {
            routes.push(prefix || '/')
          }
        }
        return routes
      ...
      return { content: [{ type: 'text', text: `Rotas existentes: ${routes.join(', ')}` }] }
    }
    default:
