...
const USERS = [
  { email: 'cadastro@conexao.com.br', pass: 'Conexao@2026', name: 'cadastro' },
  { email: 'consultor@conexao.com.br', pass: 'Conexao@2026', name: 'consultor' },
];
...
function getOrLogin(vu) {
  ...
  const res = http.post(
    ...
    { headers: { 'Content-Type': 'application/json', 'apikey': ANON_KEY }, tags: { name: 'auth_login' } }
  );
  if (res.status === 200) {
    tokens[vu] = { token: res.json('access_token'), uid: res.json('user.id'), email: user.email };
    return tokens[vu];
  }
  return null;
}
...
export default function (data) {
  ...
  group('Dashboard queries', () => {
    ['em_analise','dados_enviados','aprovado','reprovado','em_correcao','link_gerado'].forEach(s => {
      const r = http.get(`${BASE_URL}/rest/v1/cadastros?status=eq.${s}&select=id&limit=0`, { headers: h, tags: { name: 'dashboard_count' } });
  ...
  group('List cadastros', () => {
    const r = http.get(`${BASE_URL}/rest/v1/cadastros?select=id,tipo_pessoa,status,colaborador,lead_nome,created_at,codigo_cliente&order=created_at.desc&limit=20&offset=0`, { headers: h, tags: { name: 'list_cadastros' } });
  ...
  group('Clientes view', () => {
    const r = http.get(`${BASE_URL}/rest/v1/clientes?select=id,nome,cpf,status,codigo_cliente,created_at&order=created_at.desc&limit=20`, { headers: h, tags: { name: 'clientes_view' } });
  ...
  group('Notificacoes polling', () => {
    const r = http.get(`${BASE_URL}/rest/v1/notificacoes?usuario_id=eq.${data.tokens[user.email + '_uid'] || 'none'}&select=id,titulo,mensagem,lida,created_at,dados&order=created_at.desc&limit=100`, { headers: h, tags: { name: 'notificacoes_poll' } });
    check(r, { 'notificacoes poll ok': (res) => res.status === 200 });
  });

  // Scenario 5: RPC - is_admin_or_super
  group('RPC is_admin_or_super', () => {
    const r = http.post(`${BASE_URL}/rest/v1/rpc/is_admin_or_super`, '{}', { headers: hJson, tags: { name: 'rpc_admin_check' } });
  ...
  group('RPC verificar documento duplicado', () => {
    const r = http.post(
      ...
      JSON.stringify({ documento_texto: '123.456.789-00', tipo_documento: 'cpf' }),
  ...
  group('Profiles query', () => {
    const r = http.get(`${BASE_URL}/rest/v1/profiles?select=id,email,nome,ambiente,role`, { headers: h, tags: { name: 'profiles_list' } });
    check(r, { 'profiles ok': (res) => res.status === 200 });
  });

  // Scenario 8: Notificacoes templates
  group('Notificacao templates', () => {
    const r = http.get(`${BASE_URL}/rest/v1/notificacoes_templates?select=*&ativo=eq.true`, { headers: h, tags: { name: 'notif_templates' } });
