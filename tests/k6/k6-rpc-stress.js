...
function login() {
  ...
  return res.status === 200 ? { token: res.json('access_token'), uid: res.json('user.id') } : null;
}
...
export default function () {
  ...
  group('RPC limpar_links_expirados', () => {
    ...
    const res = http.post(`${BASE_URL}/rest/v1/rpc/limpar_links_expirados`, '{}', { headers: h });
  ...
  group('RPC is_admin_or_super', () => {
    const start = Date.now();
    const res = http.post(`${BASE_URL}/rest/v1/rpc/is_admin_or_super`, '{}', { headers: h });
    rpcDuration.add(Date.now() - start);
