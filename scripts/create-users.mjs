...
const users = [
  { email: 'hevertoneduardoperes@gmail.com', password: '@#Khen741963', nome: 'Heverton Peres',    role: 'admin' },
  { email: 'cadastro@conexao.com.br',         password: 'Conexao@2026', nome: 'Cadastro Conexão', role: 'editor' },
  { email: 'consultor@conexao.com.br',        password: 'Conexao@2026', nome: 'Consultor Conexão',role: 'viewer' },
  { email: 'ti@conexao.com.br',               password: 'Conexao@2026', nome: 'TI Conexão',       role: 'admin' },
];

for (const u of users) {
  const { rows: existing } = await pool.query(
    'SELECT id, email FROM auth.users WHERE email = $1', [u.email]
  );
  if (existing.length > 0) {
    console.log(`✓ ${u.email} — já existe (role atual: via trigger)`);
    // Garante que o role está correto
    await pool.query('UPDATE public.profiles SET role = $1, nome = $2 WHERE id = $3',
